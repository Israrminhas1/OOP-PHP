...
$currentuserblogs= new blogs();
$id=$_SESSION['loginid'];
$result_fetch=$currentuserblogs->getUserBlogs($id);
...
<body>
 ...
 ?>
    ...
    <script>
        ...
        let myuserid = <?php echo $_SESSION['loginid'];?> ;
        let editperm = <?php echo $_SESSION['permissions']['EditBlog'];?> ;
        let deleteperm = <?php echo $_SESSION['permissions']['DeleteBlog'];?> ;
    console.log("myid", myuserid,editperm,deleteperm);
    $.ajax({
                url: 'blogfile.php',
                type: 'get',
                success: function (res) {
                    const parsed = JSON.parse(res);
                    console.log(parsed);
                    var index = parsed.findIndex(p => p.userid == myuserid);
                    console.log(index);
    ...
    for (i = 0; i < tr.length; i++) {
      ...
      td1 = tr[i].getElementsByTagName("td")[3];
        ...
        editbut = td1.getElementsByTagName("button")[1];
if (deleteperm===0) {
    td1.getElementsByTagName("button")[0].style.display="none";
      }
      if (editperm===0) {
    td1.getElementsByTagName("button")[1].style.display="none";
      }
